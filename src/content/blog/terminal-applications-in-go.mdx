...
While we could handle this in the parent model, BubbleTea encourages the use of nested models for these types of behaviors.

In our main model, let's embed another `tea.Model` called `selector` which is also a tea model.


Let's build a second model called `Selector` that expects a set of keypresses, and updates it's internal state. It'll render a set of choices.
